...
| `/ideas` or `/ideas ls` | List the queue: doing, do (best first), maybe, inbox | **No** |
| `/ideas ls done` · `ls -a` · `ls here` | List one lane, every lane, or only this project | **No** |
| `/ideas cat 12` | Show idea #12 in full: brief, model, notes | **No** |
| `/ideas rm 12 14` | Delete ideas for good | **No** |
| `/ideas done 12 shipped it` | Mark an idea done, with a note. Also `start`, `reopen`, `drop` | **No** |
...
| `/ideas sort` | Triage the inbox now and show the queue. You do not have to: `go` triages when it must. | Yes, briefly |
| `/ideas <question>` | Ask about your ideas, e.g. "which ones fit in an hour?" | Yes, briefly |

The verb comes first, as in a shell. `/ideas all` is a command, not a view: to see every lane, use `/ideas ls -a`. `/ideas help` shows the usage. In the plugin menu the commands also appear as `/ideamine:idea` and `/ideamine:ideas`. Both forms work.

`go` does not ask questions. The queue puts the best `do` ideas first, then the best `maybe` ideas. Ideas that the triage marks `skip` stay out of the queue until you `reopen` or `rm` them.

You save ideas from any session, so the project that ideamine records is only the folder you were in. That can be a scratch folder that is gone. For this reason, `go` and `all` let Claude judge by the text which ideas fit the current chat, and where to build each one. `go` builds in the current project when the idea fits it. Else it uses the recorded folder if that folder exists, or finds the project that the idea is about. If it cannot find the project, it stops and tells you.
...
/ideas ls|cat|rm ──► same hook, answers locally
